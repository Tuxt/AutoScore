    ...
    for line in data:
        ...
        if line.startswith('X:'):
            if data_line != '':
                ...
                # Delete multiples bars  | |
                phrases = data_line.split('|')
                chunks = [ phrase.split() for phrase in phrases ]
                phrases = [ ' '.join(chunk) for chunk in chunks if len(chunk) > 0 ]
                data_line = ' | '.join(phrases)
